...
export default function VillaReservation({
  ...
  const [selectedPaymentType, setSelectedPaymentType] = useState<PaymentType>(PaymentType.FULL);
  ...
  const handleReservation = async () => {
    ...
    if (!priceCalculation || !paymentCalculation) {
      alert('Fiyat hesaplanamadı. Lütfen daha sonra tekrar deneyin.');
      return;
    }

    // İletişim bilgilerini kontrol et
    if (!contactInfo) {
      alert('Lütfen iletişim bilgilerinizi girin ve onay kutularını işaretleyin.');
      return;
    }
    ...
    if (!contactInfo.acceptTerms) {
      alert('Lütfen kiralama sözleşmesini kabul edin.');
      return;
    }
    
    if (!contactInfo.acceptPrivacy) {
      alert('Lütfen KVKK aydınlatma metnini kabul edin.');
      return;
    }
    ...
    try {
      ...
      const response = await fetch('/api/reservations', {
        ...
        body: JSON.stringify({
          ...
          paymentOption: paymentCalculation.paymentOption,
          advancePayment: paymentCalculation.currentPayment,
          remainingAmount: paymentCalculation.remainingPayment,
          // İletişim bilgilerini doğrudan contactInfo state'inden al
          contactInfo 
        }),
      });
      ...
      alert(`Rezervasyon talebiniz başarıyla oluşturuldu. ${contactInfo.fullName} adına ${new Date(checkinDate).toLocaleDateString('tr-TR')} - ${new Date(checkoutDate).toLocaleDateString('tr-TR')} tarihleri için rezervasyonunuz alındı.`);
      
      // Rezervasyon başarılı sayfasına yönlendir
      window.location.href = `/reservation-success/${result.reservationId}`;
      
    } catch (error) {
      console.error('Rezervasyon hatası:', error);
      alert(error instanceof Error ? error.message : 'Rezervasyon işlemi sırasında bir hata oluştu. Lütfen daha sonra tekrar deneyin.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  // Hata ayıklama için değerleri konsola yazdır
